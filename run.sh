...
if ! psql -h localhost -p 5432 -U "$DB_USER" -tAc "SELECT 1 FROM pg_roles WHERE rolname='$DB_USER'" | grep -q 1; then
    echo "⚠️ Роль '$DB_USER' не существует. Создаем её..."
    if ! psql -h localhost -p 5432 -U "$DB_USER" -c "CREATE ROLE $DB_USER WITH SUPERUSER LOGIN PASSWORD 'postgres';"; then
...
if psql -h localhost -p 5432 -U "$DB_USER" -lqt | cut -d \| -f 1 | grep -qw "$DB_NAME"; then
    echo "⚠️ База данных '$DB_NAME' уже существует. Удаляем её..."
    dropdb -h localhost -p 5432 -U "$DB_USER" "$DB_NAME"
fi
...
psql -h localhost -p 5432 -U "$DB_USER" -d "$DB_NAME" -f "$SQL_DIR/01_create_tables.sql"

echo "📥 3. Импорт данных..."
psql -h localhost -p 5432 -U "$DB_USER" -d "$DB_NAME" -f "$SQL_DIR/02_import_data.sql"

echo "🔍 4. Выполнение пунктов задания..."
while IFS= read -r -d '' script; do
    echo "🔄 Выполнение $(basename "$script")..."
    base_name=$(basename "$script" .sql)
    psql -h localhost -p 5432 -U "$DB_USER" -d "$DB_NAME" -f "$script" > "$OUTPUT_DIR/${base_name}_results.txt"
